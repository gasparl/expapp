...
// miscellaneous subject data [ref31]
const misc = {
...
// code to execute when the web app is entirely loaded [ref9]
document.addEventListener("DOMContentLoaded", function() {

    // check browser compatibility with relevant JavaScript methods [ref11]
    try {
    ...
    // misc.browser_name may be used to restrict browser types [ref54]
...
// get URL query parameters: language, device, etc. [ref12]
const check_params = function() {
    ...
    // check whether it's demonstration version [ref53]
...
const load_language = function() {
    // load texts in the given language [ref17]
    ...
    lg_script.onload = function() {
        ...
        // insert texts of the chosen language into HTML [ref17]
    ...
    // if the specified language file not found, default to English [ref16]
...
const ready_texts = function() {
    ...
    // disabling pasting or dropping text to the feedback text area element [ref34]
...
const consent_submit = function() {
    ...
    // go to the preliminaries division unless wrong device is detected [ref15]
    if (misc.device === 'mobile') {
        if (!is_mobile()) {
...
const set_images = function() {
    ...
    // set up drag and drop mechanism [refdrag]
...
const set_screen = function() {
    ...
    // warn if user exits fullscreen [ref21]
    fullscreen_change(() => {
...
const prelim_submit = (() => {
    // "clicked_once" to keep track of whether the user tried to proceed already [ref36]
...
// monitor clicks on attention checker item and remove on three clicks completed [ref37]
...
const prep_file = function() {
    ...
    return (subject_results + JSON.stringify(misc)); // JSON.stringify for appending miscellaneous data as JSON [ref31]
};

// have the file downloaded manually, via pop-up [ref28]
...
// store partial interim data during the experiment [ref22]
...
// store final full data at the end of the experiment [ref22]
const upload_final = function() {
    ...
    fetch('./store_main.php', {
        ...
        .then(response => response.text())
        .then(echoed => {
            // in case of successful server connection [ref27]
            console.log(echoed);
            document.documentElement.style.cursor = 'auto';
            if (echoed.startsWith("http")) {
                // in case of message indicating successful file saving [ref29]
...
// if the data was not saved successfully [ref28]
