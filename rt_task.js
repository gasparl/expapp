...
const init_block = function() {
    ...
    if (block_number > 2) {
        ...
        if (block_number === 1) { // if first block: practice
            phase = 'practice';
            practice_rts = {}; // for collecting RTs during practice, for evaluation [ref44]
        } else {
            phase = 'main';
...
// evaluate validity of practice responses [ref44]
...
// generate trial items for any given block in the response time task [ref41]
...
const init_trials = function() {
    ...
    start_listener();
    DT.loopOn(); // start recursive RAF calls [ref49]
};
    ...
    if (misc.device === 'mobile') {
        ...
        [resp_left, resp_right].forEach(elem => {
            ...
            elem.ontouchstart = function(evt) {
                ...
                touch_wrapup(elem);
                // restart inactivity checker [ref48]
                countdown_off();
                countdown();
                setTimeout(() => {
        ...
        document.onkeydown = function(e) {
            ...
            if (e.key === " " ||
                ...
                // enter fullscreen, just in case it has been escaped [ref20]
                fullscreen_on();
...
const next_trial = function() {
    ...
    if (block_items.length > 0) {
        setTimeout(function() {
            // setting the timer for inactivity (10 sec) [ref48]
            countdown();

            // execute simulated keypresses if activated [ref52]
            if (typeof sim_user === "function") {
                sim_user();
            }

            trial_number++;
            // get trial information from the pre-generated object [ref42]
            ...
            // execute code (including stimulus display command) in the next RAF call [ref51]
            DT.display(function(timestamp) {
                // display stimulus in the upcoming frame (using the trial info content [ref42])
        ...
        DT.loopOff(); // stop recursive RAF calls [ref50]
        init_block();
    }
};
...
const trial_listener = function() {
    if (misc.device === 'mobile') {
        ...
        document.onkeydown = function(e) {
            if (e.key === keys.left || e.key === keys.right) {
                ...
                // feedback about wrong key pressed [ref46]
                document.getElementById('info').style.display = 'block';
                document.getElementById('info').innerHTML = tt.key_astray;
                // store info about wrong key pressed, also indicated by preceding exclamation mark [ref45]
                if (!item_x.correct) {
                    item_x.correct = '!';
                }
    ...
    if (phase === 'practice' || item_x.ssd !== 0) {
        response_window = setTimeout(() => {
            ...
            if (phase === 'practice') {
                // exceeded time limit during practice [ref43]
...
const process_response = function(resp_event, resp_side) {
    ...
    // disable inactivity checker [ref48]
    countdown_off();
    ...
    item_x.trust = resp_event.isTrusted; // whether the keypress was simulated [ref47]
    ...
    if (phase === 'practice' && (!correct_key)) {
        // wrong response during practice [ref43]
...
// warning about wrong key or too slow (i.e., no timely) response [ref43]
...
// assign headers for the eventual results table [ref30]
let subject_results = [
...
const store_response = function() {
    ...
    // if practice, collect responses for evaluation per each direction [ref44]
    if (phase === 'practice') {
    ...
    setTimeout(() => {
        ...
        // store response details to the in the results table [ref30]
        subject_results += [
        ...
        // upload partial data after each 10 trials (for dropout analysis) [ref24]
...
// counter for inactivity and potential timing out [ref48]
