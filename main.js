...
// code to execute when the web app is entirely loaded [#8]
document.addEventListener("DOMContentLoaded", function() {

    // check browser compatibility with relevant JavaScript methods [#10]
    try {
    ...
    // misc.browser_name may be used to restrict browser types [#50]
...
// get URL query parameters: language, device, etc. [11#]
const check_params = function() {
    ...
    // check whether it's demonstration version [#49]
...
const consent_submit = function() {
    ...
    // go to the preliminaries division unless wrong device is detected [#14]
    if (misc.device === 'mobile') {
        if (!is_mobile()) {
...
const set_screen = function() {
    ...
    // warn if user exits fullscreen [#20]
    fullscreen_change(() => {
...
const prep_file = function() {
    ...
    return (subject_results + JSON.stringify(misc)); // JSON.stringify for appending miscellaneous data as JSON [#30]
};

// have the file downloaded manually, via pop-up [#27]
...
// store partial interim data during the experiment [#21]
...
// store final full data at the end of the experiment [#21]
const upload_final = function() {
    ...
    fetch('./store_main.php', {
        ...
        .then(response => response.text())
        .then(echoed => {
            // in case of successful server connection [#26]
            console.log(echoed);
            document.documentElement.style.cursor = 'auto';
            if (echoed.startsWith("http")) {
                // in case of message indicating successful file saving [#28]
