...
// store the times and names of division ("page") switches [ref5]
const div_switches = {};
...
// alert to confirm or cancel unload (i.e., leaving the page by navigating away or closing the tab) [ref18]
...
// listen for change in fullscreen mode [ref21]
...
// check if the user's device is mobile [ref14]
...
const get_radio = function(rad_name) {
    const rad_val = document.querySelector('input[refame=' + rad_name + ']:checked');
...
// function to switch divisions (virtual pages) [ref5]
const switch_div = function(current, next, wait = 0, fulls = true) {
    ...
    // record the (upcoming) division name and the current time [ref6]
    div_switches['p' + Object.keys(div_switches).length + '_' + next] = Math.round(performance.now() / 600) / 100 + wait;
    upload_interim();
};


/* Drag and Drop mechanism functions [ref38] */
...
// gets current geo position [ref39]
...
// setting audiovisual media restrictions [ref40]
...
// calculating the similarity between any two given texts [ref35]
